...
```
podman run --rm -it \
-v $IMAGE_DIR:/eib eib:dev build \ # I configured IMAGE_DIR as an env variable in .bashrc so I don't have to remember doing it // export IMAGE_DIR="/home/suse-user/edge-image-builder/eib"
--config-file eib-slm-k3s.yaml \   # The path for config file is relative to the config dir
...
```https://github.com/suse-edge/edge-image-builder/blob/main/docs/testing-guide.md
...
EIB is great to simplify ZTP workflows like the one used in SUSE ATIP 3.0 based in Metal3 and CAPI, you can prepare your images with all the configs necessary and just deploy them using CAPI and clusterctl or via Rancher with Fleet in a GitOps based flow.
